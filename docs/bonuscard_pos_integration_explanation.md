...
4. Continuous purchase validation
   ...
   - Clears `bonuscard_pending_codes` after a Validate that included them succeeds, or after a Bonuscard business error response (`errorCode` set, excluding customer-lock / error code 2 — so rejected codes are not resent on later cart edits). Precondition failures, service-unavailable payloads without `errorCode`, transport/RPC exceptions, and lock errors keep pending codes for retry. Lock recovery clears transaction state but preserves pending codes for the retry Validate. Stale discarded concurrent results also keep codes until a final outcome
