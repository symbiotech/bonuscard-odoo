...
1. Customer selected in POS
   ...
   - `bonuscard_odoo/static/src/app/bonuscard_partner_search_patch.js` extends POS customer search to include Bonuscard recruitment code and internal id (including app barcode decode) alongside the standard Odoo fields (phone search is already provided by Odoo POS)
   - POS import links a Bonuscard customer when the query matches recruitment code, internal id, app member barcode (`999000` + padded id + EAN check), phone (national / trunk-0 / E.164 equivalent), or email; short fuzzy single API hits (e.g. `0724`) are rejected
...
4. Continuous purchase validation
   ...
   - Keeps `bonuscard_pending_codes` after a successful Validate so later Validates and Finalize can send the same `codes` (Finalize must match the last Validate). Clears them after Finalize succeeds, on cancel / purchase-state clears, or after a Bonuscard business error response (`errorCode` set, excluding customer-lock / error code 2 — so rejected codes are not resent). Precondition failures, service-unavailable payloads without `errorCode`, transport/RPC exceptions, and lock errors keep pending codes for retry. Lock recovery clears transaction state but preserves pending codes for the retry Validate. Stale discarded concurrent results also keep codes until a final outcome
   ...
   - Validation failures (business or transport) show sticky warnings that persist until the cashier dismisses them
...
5. Payment confirmation
   ...
   - This sends the stored `transactionIdentifier`, `checkoutItems`, and `bonuscard_pending_codes` (same codes as the last Validate) to Bonuscard
   - Finalization is retried once on failure; on success the POS clears runtime transaction fields (`bonuscard_transaction_id`, `_bonuscardCandidateTxId`, `bonuscard_checkout_items`, `bonuscard_pending_codes`) via `_clearBonuscardRuntimeTransactionState`
   ...
   - **Customer Account → sale order** (`pos_order_to_sale_order` with Create Sale Order when paying with Customer Account): no `pos.order` is synced, so `preSyncAllOrders` never runs. The companion module `bonuscard_pos_order_to_sale_order` (auto-installed when both parents are present) calls `_applyBonuscardAuditAfterPayment` after a new `finalizeSaleOrderFromPos` conversion. The Actions → Create Order button path is unchanged (still Cancel on delete if a transaction is pending).
...
- `res.partner` extension
  ...
  - `_filter_bonuscard_customers_for_pos_query` accepts recruitment-code, internal-id, app-barcode, phone (national / trunk-0 / E.164 equivalent via `_phones_equivalent`), or email matches for POS import; short fuzzy single API hits are rejected
  - Adds `action_register_to_bonuscard`, `action_refresh_bonuscard_status`, and `action_clear_bonuscard_link`
  - Adds `import_partner_from_bonuscard_for_pos` to create or link an Odoo partner from Bonuscard when POS search finds no local match; requires a recruitment-code, internal-id/app-barcode, phone, or email match on the Bonuscard API result (app barcodes also retry SearchCustomers with the extracted id)
