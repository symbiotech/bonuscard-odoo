...
4. Continuous purchase validation
   ...
   - Clears `bonuscard_pending_codes` after a Validate that included them succeeds, or after a Bonuscard business error response (`errorCode` set — so rejected codes are not resent on later cart edits). Precondition failures, service-unavailable payloads without `errorCode`, and transport/RPC exceptions keep pending codes for retry. Stale discarded concurrent results and lock-recovery retries also keep codes until that final outcome
