...
* Companion module ``bonuscard_pos_order_to_sale_order`` finalizes Bonuscard when Customer Account payment creates a sale order via ``pos_order_to_sale_order`` (auto-install)
...
``default_code``) cannot be marked as in the catalog. On the POS product
grid, catalog products show a small Bonuscard mark (pink ``B`` icon) on the
tile. The POS product info popup (long-press a product tile) shows the
current Bonuscard catalog status for single-variant products.
...
search query matches the Bonuscard recruitment code, internal id, app member
barcode, phone, or email on an API result. App barcodes are EAN-13 values of the
form ``999000`` + zero-padded internal id + check digit (e.g. ``9990009763581``
...
* When ``pos_order_to_sale_order`` converts a Customer Account payment into a
  sale order (no ``pos.order`` sync), companion module
  ``bonuscard_pos_order_to_sale_order`` finalizes via the same post-payment
  helper (auto-installed when both parents are present)
