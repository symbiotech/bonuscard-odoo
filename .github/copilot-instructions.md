...
- POS customer import (`import_partner_from_bonuscard_for_pos`) accepts a Bonuscard `SearchCustomers` result when the cashier's query matches recruitment code, internal id, app member barcode (`999000` + padded id + EAN check digit), phone (same equivalence as above), or email. App barcode queries also retry SearchCustomers with the extracted id. A short fuzzy API hit (for example `0724` matching internal id `724`) is rejected.
...
| `bonuscard_pos_order_to_sale_order/README.rst` | Companion: Finalize when Customer Account creates a sale order | Bridge behavior or install conditions change |
